...
import { mockedUser, mockedUserLogin, newClient, newSale, saleId } from "../../mocks"
...
describe("/Sales", ()=> {
    ...
    test("POST /sales - Must be able to create a sale",async () => {
        ...
        await request(app).post("/clients").set("Authorization", token).send(newClient)
        const getClient = await request(app).get("/clients").set("Authorization", token)
        
        newSale.clients = getClient.body[0].id

        const response = await request(app).post("/sales").set("Authorization", token).send(newSale)
    ...
    test("GET /sales -  Must be able to list all sales",async () => {
        ...
        const response = await request(app).get("/sales").set("Authorization", token)
    ...
    test("GET /sales/:id - should not be able to update nother sale without invalid saleId",async () => {
        ...
        const token = `Bearer ${responseLogin.body.token}`
        const response = await request(app).get(`/sales/123`).set("Authorization", token)
    ...
    test("GET /sales/:id - must be able get a sale",async () => {
        ...
        const response = await request(app).get(`/sales/${saleId.id}`).set("Authorization", token)
